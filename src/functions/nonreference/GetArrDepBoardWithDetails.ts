...
const fnName = "GetArrDepBoardWithDetailsAsync";

export interface IArgs extends IArgsBase {
  args: Optional<GetArrDepBoardWithDetailsAsyncArgs> & { crs: string };
}

export const fn = async ({ apiKey, args }: IArgs) => {
  const defaultValues: Optional<GetArrDepBoardWithDetailsAsyncArgs, "crs"> = {
    filtercrs: "",
    timeOffset: 0,
    filterType: "to",
    numRows: 150,
  ...
  return Object.values(result?.[0])?.[0] as GetArrDepBoardWithDetailsAsync;
};
